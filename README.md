...
- **[Surface Studio](predict-surface-studio/)** - live 3D viewer of the SVI volatility surface streamed from on-chain `OracleSVIUpdated` events, with arbitrage-free butterfly + calendar checks.
- **[Vol-Arb Bot](predict-vol-arb/)** - cross-venue arbitrage between DeepBook Predict and Polymarket BTC binary smiles. Trades the spread when implied vol disagrees by more than a configurable edge.
- **[Telegram Quick-Predict Bot](predict-tg-bot/)** - one-line trading from any Telegram chat (`/up 70k 15m 100usdc`). Custodial wallet per user, auto-creates a `PredictManager`, faucets dUSDC on first use, redeems on settlement.

The three pieces are independent but share the same SVI core, so the surface viewer shows you exactly the implied vol the bot is trading against.
...
Arbitrage-free conditions checked live:
- **Butterfly**: `g(k) = (1 - k*w'/(2w))^2 - (w'^2/4)*(1/w + 1/4) + w''/2 >= 0`
- **Calendar**: `w_T2(k) >= w_T1(k)` for all `T2 > T1`

## Network
Sui testnet today (DeepBook Predict ships from the `predict-testnet-4-16` branch). Mainnet redeploy planned when the protocol ships.
